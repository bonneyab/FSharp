...
module Problem40 =
    let private generateIrrationalNumber max =
        let rec _generateIrrationalNumber (c:int) (m:int) (acc:string) =
            match acc with
            | i when i.Length >= m -> i
            | i -> _generateIrrationalNumber (c + 1) m (acc + c.ToString())
        _generateIrrationalNumber 1 max ""

    let getDigitsFromIrrationalNumber digits =
        let max = digits |> Seq.max
        let irrationalArray = (generateIrrationalNumber max).ToCharArray()
        digits 
